...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            # following the pseudo code from reference paper
            a_1 = np.empty(shape=(3,1))

            a_1[0] = (self.goal.row - self.start.row) / c_min
            a_1[1] = (self.goal.col - self.start.col) / c_min
            a_1[2] = 0

            # extracting a first column of the identity matrix
            I_1 = np.atleast_2d(np.eye(3)[0:3, 0]).T
            
            # following equation from ref paper
            M = np.dot(a_1, I_1.T)

            # reference from: "https://numpy.org/doc/stable/reference/generated/numpy.linalg.svd.html"
            U, S, V_T = np.linalg.svd(M, full_matrices=True)
            V = V_T.T

            diag = np.diag([1, 1, np.linalg.det(U)*np.linalg.det(V)])
            ...
            rad = 2
            size = (num,rad+2)
            u_temp = np.random.normal(0, 1, size)                               # ref from: "https://numpy.org/doc/stable/reference/random/generated/numpy.random.normal.html"
            norm = np.linalg.norm(u_temp, axis = -1,keepdims = True)
            u = u_temp/norm
            
            # sampling a point from x_ball with centre zero and radius one
            x_ball = np.zeros((3, 1))
            x_ball[0:2, :] = np.atleast_2d(u[0,:rad]).T
